...
header-includes:
  ...
  - '\usepackage{caption}'
  - '\captionsetup[figure]{font={normalfont,normalsize},labelfont=bf,textfont=normalfont,justification=centering}'
...
\captionof{figure}{Diagramme des cas d'usage}
&
\vspace{-10cm}
...
Les diagrammes d'activité présentent l’enchaînement des actions sur l'application et les choix possibles à chaque étape pour l'utilisateur. Le diagramme général montre l’accès aux principales fonctionnalités depuis l’accueil ; les suivants les détaillent.
...
\captionof{figure}{Diagramme d'activité : parcours utilisateur global}
\end{minipage}
```
...
\captionof{figure}{Diagramme d'activité : gestion du profil personnel}

\vspace{1cm}

\includegraphics[width=10cm]{diagramme_activite_livre}

\captionof{figure}{Diagramme d'activité : consultation d'un livre}
\end{minipage}
```
...
Le travail préliminaire de construction de notre projet nous a tout d'abord permis de nous connaître entre membres du groupe, d'apprendre à nous écouter et de voir les capacités de chacun. L'avantage de notre groupe est que nous voulons tous progresser en informatique grâce à ce projet en nous informant de notre côté et surtout en nous transmettant les connaissances les uns aux autres. 


Le dialogue intergroupe a permis d'expliciter et de déployer le sujet. Avec l'imagination et les connaissances de chacun, nous avons pu mettre sur pied les différents diagrammes qui sont présentés dans ce rapport. Cela nous a permis de donner une première structure à notre projet et nous fixer des objectifs. Nous avons l'intention d'affiner et de compléter nos connaissances techniques durant la suite de ce projet.


Ce rapport nous a permis de découvrir l''architecture en couches avec les fonctionnalités nécessaires pour coder l'application demandée (controller, Dao, Service) et nous a obligé à réfléchir sur les moyens pour l'optimiser (nombre de requêtes à OpenLibrary, stockage des donnée intéressantes).
