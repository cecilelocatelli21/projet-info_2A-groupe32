...
Les diagrammes d'activité présentent l’enchaînement des actions sur le site et les choix possibles à chaque étape pour l'utilisateur. Le diagramme général montre l’accès aux principales fonctionnalités depuis l’accueil ; les suivants les détaillent.
...
Notre planning se découpe en deux grandes phases : la phase d'analyse, de fin août à fin septembre, suivie de la phase de code, d'octobre à début novembre.

\noindent
\begin{tabular}{@{}p{0.60\textwidth}@{\hspace{0.03\textwidth}}p{0.35\textwidth}@{}}
\vspace{0pt}
\centering
\includegraphics[width=\linewidth]{Gantt.png}

\small Figure 11 – Diagramme de Gantt
&
\vspace{0pt}
\small
Nous commençons par étudier l'API Open Library, puis nous réalisons
l'ensemble de la modélisation UML afin de bien comprendre comment notre
application va s'articuler et comment la construire.

Nous avons choisi de développer les fonctionnalités par ordre de
difficulté croissante : d'abord les classes de base, puis
...
temps du fait de sa complexité.

Les tests unitaires et le frontend interviendront en dernier, le
frontend n'étant pas prioritaire, sera développé uniquement s'il nous
reste du temps.
\end{tabular}
...
Nous avons construit l'application en partant des **Business Object** : ce sont eux qui définissent les entités manipulées (attributs, sans logique métier). Les classes Controller/Service/DAO sont construites ensuite autour de ces BO, en couches (`controller → service → dao`) ; ces classes ne sont pas systématiquement détaillées ici, en particulier le Controller, couche de routage sans logique propre, est illustré dans les diagrammes de séquence plutôt que dans les diagrammes de classes. Nous présentons ci-dessous un exemple pour illustrer cette articulation : **les critiques**.
...
\end{center}
-->
`CritiqueDao` fait le lien entre les objets `Critique` en mémoire et la table correspondante : `find_by_livre` récupère toutes les critiques d'un livre (toutes éditions confondues), les autres méthodes sont des opérations classiques de persistance. `LikeDao` en est l'équivalent pour la table `Like` : `count_by_critique` compte les likes/dislikes d'une critique, `exists` vérifie qu'un utilisateur n'a pas déjà réagi à une critique donnée, et `update` sert lorsqu'un utilisateur bascule d'un like à un dislike ou inversement.

`CritiqueService` porte toute la logique métier, les DAO n'exécutant que des requêtes. `rediger_critique` vérifie que le texte n'est pas vide (indépendamment de toute note, les deux étant volontairement découplés). `modifier_critique` et `supprimer_critique` vérifient que l'action est bien effectuée par l'auteur de la critique ; `supprimer_critique` orchestre en plus la suppression des likes associés via `LikeDao` avant de supprimer la critique elle-même. `consulter_critiques_livre` combine `CritiqueDao.find_by_livre` et `LikeDao.count_by_critique` pour renvoyer les critiques accompagnées de leur nombre de likes. Enfin, `liker` prend un paramètre booléen `aime` : une seule méthode gère donc le like et le dislike, en s'appuyant sur `LikeDao.exists` pour éviter les doublons.

<!--
Des diagrammes similaires représentent les classes relatives à l'abonnement, aux recommendations et à l'authentification. Nous n'avons pu les présenter ici faute de place.
-->
