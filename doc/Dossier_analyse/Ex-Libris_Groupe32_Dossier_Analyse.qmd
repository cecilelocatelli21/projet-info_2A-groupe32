...
La première étape est de récupérer les informations pertinentes fournies par **OpenLibrary** sur ce livre (titre, auteur, date, cover, etc. – la liste précise reste à définir), en se basant sur l’`id_work` considéré.

Ensuite, il s’agit de fournir les informations propres à notre application Ex-Libris.

Pour cela, le **Service** va utiliser l’`id_work` pour rechercher dans la base de données si un livre a déjà été enregistré associé à celui-ci. Si oui, il récupère l'`id_livre` correspondant.

Sinon, il lui proposera d'être le premier à créer une fiche lecture.

À partir de cet `id_livre`, le **Service** va récupérer toutes les notes mises par les utilisateurs à ce livre, pour ensuite en faire la moyenne.
...
À noter que dans cette séquence, si le livre n’existe pas dans la base de données Ex-Libris, il n’est pas ajouté à cette base. En effet, il a été convenu par l’équipe projet qu’un ajout ne se fait qu’au moment où un utilisateur enregistre le livre dans sa bibliothèque.
...
La première étape est de récupérer la liste des livres lus par cet utilisateur (`id_livre`), ainsi que les notes associées à chacun de ces livres (`note`).
...
Le **Service** va donc essayer d’identifier dans la liste de livres de l'utilisateur les auteurs les plus appréciés. S’il trouve des candidats, il va interroger **OpenLibrary** pour rechercher d’autres livres des mêmes auteurs.

On pourra éventuellement filtrer parmi ces résultats ceux qui ont les meilleures notes dans Ex-Libris, mais cela n’apparaît que comme commentaire sur le diagramme de séquence pour ne pas alourdir le schéma.
...
Dans un premier temps, le **Service** récupère la liste des utilisateurs auxquels celui-ci est abonné (`id_followed` dans la table `Abonnement`).
...
À la fin de l'exécution des algorithmes de recommandations au niveau du **Service**, celui-ci va faire remonter une liste des livres recommandés pour l'utilisateur (`titre`, `auteur`, etc.), pour les faire afficher par le **Contrôleur**.
...
Un autre type d'algorithme envisagé serait de recouper les livres lus et bien notés entre l'utilisateur initial et d'autres utilisateurs de l'application. Nous obtiendrions alors des utilisateurs avec des goûts similaires à l'utilisateur initial, ce qui permettrait de faire de nouvelles suggestions à l'utilisateur.
